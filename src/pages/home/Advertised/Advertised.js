...
const Advertised = () => {
    ...
    const { isLoading, error, data, refetch } = useQuery('advertised', () => fetch(`http://localhost:5000/advertisedcars`).then(res => res.json()))
    if (isLoading) {
    ...
    return (
        <>
            {data?.length > 0 &&
                <div className='my-5'>
                    <h1 className='text-center pt-5 pb-3 justify-content-center'>Advertised most popular Guns! <GiChiliPepper style={{ color: "red" }} /></h1>
                    <div>
                        <Swiper
                            centeredSlides={true}
                            autoplay={{
